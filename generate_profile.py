...
def render(theme_name, colors, stats, ist_now):
    ...
    parts.append(
        f'<line x1="1" y1="38" x2="{W-1}" y2="38" stroke="{colors["border"]}"/>'
    )
    for i, c in enumerate(["dot1", "dot2", "dot3"]):
        parts.append(
            f'<circle cx="{24 + i*20}" cy="20" r="6" fill="{colors[c]}"/>'
        )
    parts.append(
    ...
    for label, value, ckey in INFO:
        ...
        if label == "__header__":
            parts.append(
                f'<text x="{INFO_X}" y="{y:.1f}" class="hdr row" {d}>'
                f"{escape(value)}</text>"
            )
            y += INFO_LH
        elif label == "__rule__":
            ...
            parts.append(
                f'<text x="{INFO_X}" y="{y:.1f}" class="sec row" {d}>'
                f"{escape(value)}</text>"
            )
            y += INFO_LH
            ...
            if label:
                parts.append(
                    f'<text x="{INFO_X}" y="{y:.1f}" class="key row" {d}>'
                    f"{escape(label)}</text>"
                )
            parts.append(
                f'<text x="{VAL_X}" y="{y:.1f}" class="{cls} row" {d}>'
                f"{escape(value)}</text>"
            )
            y += INFO_LH
        delay += 0.07
...
def main():
    ...
    for name, colors in THEMES.items():
        (out / f"{name}.svg").write_text(
            render(name, colors, stats, stamp), encoding="utf-8"
        )
